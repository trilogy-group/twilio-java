...
import static com.kandy.Assert.assertQueryStringsEqual;
import static com.kandy.Assert.assertUrlsEqual;
...
public class RequestTest {
    ...
    public void testRegionInConstructor() {
        final Request request = new Request(HttpMethod.GET, Domains.ACCOUNTS.toString(),
                "/path/to/something.json?foo=12.34#bar", "region");

        assertUrlsEqual("https://accounts.region.twilio.com/path/to/something.json?foo=12.34#bar",
                request.constructURL());

        request.setEdge("edge");
        assertUrlsEqual("https://accounts.edge.region.twilio.com/path/to/something.json?foo=12.34#bar",
                request.constructURL());
    }
